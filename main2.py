...
def validateSelect(tokens): 
    leng = len(tokens)
    if leng < 5 or tokens[2] != 'FROM':
        raise Syntax_Error("Syntax Error: INVALID SELECT")
    
    if leng == 5:
        table_name = tokens[3]
        if ',' in table_name:
            raise Unsupported_Functionality("Multiselct not supported")
        if table_name not in databases:
            raise Not_Exist(f"Table {table_name} does not exist")
        cols = validateColumns(tokens[1],table_name)
        if tokens[4] == ';':
            simple_select(cols,databases[table_name])
            return
        return
...
def validateJoin(tokens):
    if len(tokens) < 9 or tokens[4] != 'JOIN' or tokens[6] != 'ON':
        raise Syntax_Error('Syntax Error: JOIN')
    
    if tokens[8].startswith('WHERE'):
        validateWhere()
