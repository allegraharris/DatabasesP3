...
### **table definition**: 
#   table = [ columns, column, indexing, primary_key, tuple_1, tuple_2, ......, tuple_n ]
...
def parse_columns(table):
    ...
    attributes = parsed[1:len(parsed)-1].strip().split(',')
    # columns = table[0]
    # column = table[1]
    # keys = table[2]
    index = 0
    for attribute in attributes:
        ...
        if name == 'FOREIGN':
            if len(tokens) != 3 or tokens[1] != 'KEY':
                raise Syntax_Error("Syntax Error: Primary key")
            key = tokens[2][1:len(tokens[2])-1].strip()
            if key not in table[0]:
                raise Syntax_Error("Syntax Error: Column '" + key + "' does not exist")
            table[0][key][1] = 2
        if name != 'PRIMARY':
...
def eval_query():
    ...
    if optr == 3:
        ...
        insert_info = query_tokens[2].split(' ')
        ...
        if len(column) == 2 or column[1:len(column)-1].strip().isspace():
            return insert(table_name,indexing)
        attributes = column[1:len(column)-1].strip().split(',')
        for attribute in attributes:
            if attribute not in databases[table_name][0]:
                raise TABLE_EXIST(f"Column {attribute} does not exist")
            indexing.append
